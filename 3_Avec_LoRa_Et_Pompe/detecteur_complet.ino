/*******************************************************************************
 * UCA21 Board - Detecteur de flamme IR + Pompe + Alerte LoRaWAN TTN
 * 
 * Capteur IR sur D5, Buzzer sur D3, LEDs WS2812 sur D4, Relais pompe sur D2
 * Bibliotheque : IBM LMIC framework v1.5.1 + FastLED
 * 
 * Comportement :
 *   - Pas de flamme : LEDs vertes, silence, pompe OFF
 *   - Flamme detectee : LEDs alternance bleu/rouge + buzzer + pompe ON + alerte TTN
 * 
 * Note : le capteur IR donne LOW quand il detecte une flamme
 * 
 * Branchement pompe :
 *   - Fil rouge (+) -> Batterie (+) via relais COM/NO
 *   - Fil noir  (-) -> GND commun
 *   - Fil vert (signal) -> non utilise (pompe simple)
 * 
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println(F("=== UCA21 Detecteur IR + LoRa + Pompe ==="));

  pinMode(IR_PIN, INPUT);
  pinMode(BUZZER_PIN, OUTPUT);
  pinMode(RELAY_PIN, OUTPUT);
  digitalWrite(RELAY_PIN, LOW); // Pompe OFF au demarrage
  pinMode(7, OUTPUT);
  digitalWrite(7, HIGH);
